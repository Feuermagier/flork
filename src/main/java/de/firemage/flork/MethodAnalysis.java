...
public class MethodAnalysis {
    ...
    private MethodAnalysisResult analyzeMethod(CtMethod<?> method) {
        MethodAnalysisResult
            result = new MethodAnalysisResult(method.getType());
    ...
    private StatementControlFlowResult analyzeBlock(CtBlock<?> block, Map<CtVariable<?>, VariableState> variableState,
        ...
        for (var statement : block.getStatements()) {
            ...
            if (statementResult instanceof ReturnedStatementResult ||
                statementResult instanceof ExceptionStatementResult) {
    ...
    private StatementControlFlowResult analyzeStatement(CtStatement statement,
                                                        Map<CtVariable<?>, VariableState> variableState,
                                                        MethodAnalysisResult result) {
        if (statement instanceof CtReturn<?> ret) {
            ExpressionResult
                state = analyzeExpression(ret.getReturnedExpression(), variableState);
            ...
            if (local.getAssignment() != null) {
                ExpressionResult
                    state = analyzeExpression(local.getAssignment(), variableState);
                if (state.isException()) {
                    return new ExceptionStatementResult(state.getThrownException());
                } else {
    ...
    private ExpressionResult analyzeExpression(CtExpression<?> expr, Map<CtVariable<?>, VariableState> variableState) {
        if (expr instanceof CtAssignment<?, ?> assignment) {
            ExpressionResult
                rhsNullable = analyzeExpression(assignment.getAssignment(), variableState);
            ...
            return new ExpressionResult(
                variableState.getOrDefault(variable, VariableState.defaultForType(expr.getType())));
            ...
            return new ExpressionResult(new VariableState(
                VariableType.REFERENCE, new ObjectValueSet(false, true)));
        } else if (expr instanceof CtUnaryOperator<?> operator) {
            ExpressionResult
                inner = analyzeExpression(operator.getOperand(), variableState);
            ...
            ExpressionResult
                lhs = analyzeExpression(operator.getLeftHandOperand(), variableState);
            if (lhs.isException()) {
                return lhs;
            }
            ExpressionResult
                rhs = analyzeExpression(operator.getRightHandOperand(), variableState);
            if (rhs.isException()) {
                return rhs;
            }
    ...
    private ExpressionResult analyzeInvocation(CtInvocation<?> invocation,
                                               Map<CtVariable<?>, VariableState> variableState) {
        CtExpression<?> target = invocation.getTarget();
        ExpressionResult
            targetState = analyzeExpression(target, variableState);
        if (targetState.isException()) {
            ...
            if (executable instanceof CtMethod<?> method) {
                return new ExpressionResult(
                    this.resultCache.computeIfAbsent(method, this::analyzeMethod).getReturnState());
            } else {
        ...
        for (Map.Entry<CtVariable<?>, VariableState> otherEntry : other.entrySet()) {
            if (target.containsKey(otherEntry.getKey())) {
                target.put(otherEntry.getKey(),
                    new VariableState(target.get(otherEntry.getKey()), otherEntry.getValue()));
            }
        }
    }
    ...
    private VariableState literalToVariableState(CtLiteral<?> literal) {
        if (literal.getValue() == null) {
            ValueSet
                valueSet = new ObjectValueSet(true, false);
            return new VariableState(
                VariableType.REFERENCE, valueSet);
        } else if (literal.getValue().equals(Boolean.TRUE)) {
            ValueSet
                valueSet = new BooleanValueSet(true, false);
            return new VariableState(
                VariableType.BOOLEAN, valueSet);
        } else if (literal.getValue().equals(Boolean.FALSE)) {
            ValueSet
                valueSet = new BooleanValueSet(false, true);
            return new VariableState(
                VariableType.BOOLEAN, valueSet);
        } else {
            throw new UnsupportedOperationException();
